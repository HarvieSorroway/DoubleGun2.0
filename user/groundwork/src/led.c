...
void ledX_On(uint16_t ledX)//点亮指定位置的led
{
	if(UsingLED)
	{
		ledX_light(ledX);	
...
void ledX_Off(uint16_t ledX)//熄灭指定位置的led
{
	if(UsingLED)
	{
		if(ledX < 2)
...
void ledX_cmd(uint16_t ledX)//熄灭其它led的同时点亮指定位置的led
{
	GPIO_SetBits(GPIOD,GPIO_Pin_10 | GPIO_Pin_11);
	GPIO_SetBits(GPIOG,GPIO_Pin_2 
						| GPIO_Pin_3
