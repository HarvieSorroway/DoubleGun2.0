	...
	if(CAN_GetITStatus(CAN1,CAN_IT_FMP0) != RESET)
	{
		...
		switch (RxMessage.StdId)
		{
			...
			case CAN1_RX_MOTO3508_4:
			{
				...
				//moto_infomation_process(&DataCollection.Data_m3508[i],&RxMessage);//???Frame??
			...
			case CAN1_RX_MOTO2006_5_2:
			{
				...
				//moto_infomation_process(&DataCollection.Data_m6020[i],&RxMessage);//???Frame??
				moto_infomation_process(&moto_m6020_info[i],&RxMessage);
			}break;
		}	
	}
}
...
void CollectData_CAN(void)
{
	for(int i = 0;i < 4;i++)
	{
		DataCollection.Data_m3508[i] = moto_chassis_info[i];
		DataCollection.Data_m6020[i] = moto_m6020_info[i];
	}
}
