...
dbus_ControlData Dbus_ControlData;
DataFilltor dbus_filltor;
...
void RC_Init(void)
{
	...
	dbus_filltor.DataFillPointer  = DBUS_DataFilltor;
	dbus_filltor.DataSetupPointer = DBUS_DataSetup;
	
	AddFilltor(dbus_filltor);
}
...
void RemoteDataProcess(uint8_t *pData)
{
	if(pData == NULL)
	{
	 ...
	 DataCollection.Data_RC = RC_CtrlData;
	 //your control code ….
...
void dbusDataProcess(void)
{
	//运动模式选择
	switch(RC_CtrlData.rc.s1)
	{
		case 1:
			Dbus_ControlData.gimbal_mode = Gimbal_gyro;
			Dbus_ControlData.chassic_mode = Chassic_rotate;
			break;
		case 3:
			Dbus_ControlData.gimbal_mode = Gimbal_gyro;
			Dbus_ControlData.chassic_mode = Chassic_return;
			break;
		case 2:
			Dbus_ControlData.gimbal_mode = Gimbal_visual;
			Dbus_ControlData.chassic_mode = Chassic_return;
			break;
	}
	
	//底盘自旋模式方向选择
	if(Dbus_ControlData.chassic_mode == Chassic_rotate)
	{
	...
	Dbus_ControlData.leftRocker_xAxis  = (RC_CtrlData.rc.ch2 - 1024) / 660.0;
	Dbus_ControlData.leftRocker_yAxis  = (RC_CtrlData.rc.ch3 - 1024) / 660.0;
	Dbus_ControlData.rightRocker_xAxis = (RC_CtrlData.rc.ch0 - 1024) / 660.0;
	Dbus_ControlData.rightRocker_yAxis = (RC_CtrlData.rc.ch1 - 1024) / 660.0;
