...
void pid_caculate_kalman(pid_typedef*pid,float Sv_t,float Av_t,kalman_filter_type_t*kalman)
{
	pid->Sv=Sv_t;
	pid->Av=Av_t;
	pid->err=pid->Sv-pid->Av;
	
	pid->del_err_kf = kalman_filter_cali(kalman, pid->err);
	pid->del_err = pid->del_err_kf-pid->del_err_bf;
	
	pid->total_err+=pid->err;
	pid->pout=pid->Kp*pid->err;
	pid->iout+=pid->Ki*pid->err;
	pid->dout = pid->Kd*(pid->del_err);
	pid_abs_limit(&pid->iout,pid->Pid_integral_limit);
	pid->out=pid->pout+pid->iout+pid->dout;
	pid->err_last=pid->err;
	pid_abs_limit(&pid->out,pid->Pid_out_limit);
	pid->del_err_bf=pid->del_err_kf;
}
